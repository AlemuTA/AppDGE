...
server <- function(input, output, session) {
  ...
  output$perf.plot1.ui <- renderUI({
    plotOutput("perf.Plot1", height = ifelse(length(input$metrics) %in% c(1, 3) & length(input$biotype) == 1, "400px",
                                             ifelse(length(input$metrics) %in% c(1, 3) & length(input$biotype) == 2, "650px",
                                             ifelse(length(input$metrics) %in% c(2, 4), "650px",  "800px"))),
  ...
  output$Optional.Plots1.ui <- renderUI({
    nops <- sum(input$opt.plot1, input$opt.plot2, input$opt.plot3)
    height <- ifelse(nops == 3, "900px", "600px")
  ...
  output$perf.Plot1      <- renderPlot({
    ...
    if(input$sim == "Zhang"){
      ...
      validate(need(input$DE.tool  != '', 'Select DE tool(s)!'),
               need(input$metrics != '', 'Select performance comparison metric(s).'),
               need(input$biotype2 != '', 'Only mRNA are studied for GTEx RNA-seq source data'))
      biotype     <- "mRNA"
      metrics     <- metrics()
      rep.size    <- rep.size3()
      PDE         <- PDE()
    ...
    plt <- ggplot(df.sub, aes(y=mean, x=X1, group = interaction(X2, X3)))+
  ...
  output$topDEtools_note <- renderText({
    validate(need(input$DE.tool != '', 'Select DE tool(s)!'),
             ...
             need(input$rep.size %in% c(2:15, 20, 30, 40), 
                  'Numbeer of samples needs to be one of the values in (2, 3, 4, 5, 7, 10, 15, 20, 30, 40).'))
    paste0("The table displays the top ", input$k, " DE tools for a given scenario. It lists DE tools based on their FDR and TPR 
          (at a particular setting, such as number of repliccates,  variability as represented by the source datasets, ...) separately for each biotype. 
          For a given biotype and other specific settings, tools with the actual FDR at most the nominal level, (", input$thrld, 
          ") are selected and ordered by their TPR (descending order). If there are no DE tools with actual FDR below the nominal level or the number of tools
          with the actual FDR at most the nominal level are less than ", input$k, ", then DE tools with the lowest ", input$k, 
         " actual FDR will be displayed ordered by their TPR (descending order). Also, for a given scenario, DE tools with the highest", input$k, 
         " TPR are will be displayed with ascending order by their actual FDR.")
  })
  ...
  output$Optional.Plots1 <- renderPlot({ 
    #Opt.plot.1  <- Opt.plot.1()
      ...
      if(input$opt.plot1){
        ...
        opt.plots.list[["opt.plot1"]] <- ggplot(df.sub, aes(x=rep.size, y=mean, group=interaction(biotype, metrics), 
      ...
      if(input$opt.plot2){
        ...
        if(max(as.numeric(df.sub$rep.siz)) == 40) {w=1} else {w=0.25}
        opt.plots.list[["opt.plot2"]] <- ggplot(df.sub, aes(x=alpha, y=mean, group=interaction(biotype, metrics), 
                                                            colour = metrics, lty=biotype))+
          geom_line(position=position_dodge(0), size=1.3)+
          #geom_point(position=position_dodge(0), size=3)+ 
          #geom_errorbar(aes(ymin=ll, ymax=ul), position=position_dodge(0), size=0.75, width=w)+
      ...
      wtlist         <- c(input$opt.plot1,input$opt.plot2,input$opt.plot3)
...
for example using multi dimensional scale (MDS) plot or principal component analysis (PCA) of replicates, 
  ...
  output$elucidation1 <- renderUI({
    list1 <- paste0("This web tool is created for easier exploration of a simulation study on the evaluation of statistical 
                    ...
                    The performance of 13 statistical tools are reviewed with a simulation study under variety of gene-expression experiment scenarios.  
    ...
    list2 <- paste0("Three sets of simulations were performed, each starting from a different source dataset: the Zhang, NGP Nutlin, and GTEx datasets. 
                    The degree of homogeneity among the replicates in these source datasets varies, suggesting that they have different levels of intra-group 
                    biological variability, which is also reflected in the simulated gene expression data. Gene expressions are simulated containing a given 
  ...
  output$elucidation2 <- renderUI({
    list <- paste0("The first panel ('Performance of all DGE tools') shows the performance of selected (or all) DE tools from one of the three simulations for a specific performance metrics,
           ...
           are shown using bar plots with error bar on the top. Different performance metrics are included, which will be automatically calculated from 
           the sorted raw simulation results. <br/> <br/>
           ...
           and hence the mean (and the 95% interval estimates) are presented using bar plots (and error bars). The metrics are calculated at given level of nominal FDR, which can also be controlled by the user. <br/>
           ...
           proportion of true DE genes, and nominal FDR. The user can control which plot to see and the axis scales. Two other tab panels are also included showing 
                   the observed features of 3 RNA-seq datasets that were used as simulation source and features of the simulated data, respectively. These 
                   ...
                   Additionally, another tab panel 'DE Tools' is dedicated to introduce the user to the DE tools reviewed in this web tool. 
                  It includes brief description to each DE tool along with their package name and references. .")
    HTML(list)
  })
  output$elucidation3 <- renderText({
    list1 <- paste0("Alemu Takele Assefa <br/> 
                    Department of Mathematical Modeling, Statistics and Bioinformatics  <br/>
                    Faculty of Bioscience Engineering <br/> 
                    Ghent University <br/> 
                    
                    Coupure Links 653, B - 9000 Ghent, Belgium <br/> 
